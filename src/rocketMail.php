...
class rocketMail
{
    public static $view_namespace = 'rocketmail';
    ...
    {
        $file = config('rocketmail.mailables_dir').'templates.json';
        if (! file_exists($file)) {
            if (! file_exists(config('rocketmail.mailables_dir'))) {
                mkdir(config('rocketmail.mailables_dir'));
            }
    ...
    {
        return collect(config('rocketmail.skeletons'));
    }
    ...
    protected static function mailablesList()
    {
        ...
        if (! file_exists(config('rocketmail.mailables_dir'))):
            ...
            $allFiles = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(config('rocketmail.mailables_dir')));
            ...
            foreach ($args->all() as $arg) {
                ...
                if (is_array($arg)) {
                    if (isset($arg['is_instance'])) {
                        if (isset($eloquentFactory[$arg['instance']]) && config('rocketmail.factory')) {
    ...
    public static function renderPreview($simpleview, $view, $template = false, $instance = null)
    {
        ...
        if (! $template) {
            ...
            foreach ($_data as $key => $value) {
                if (! is_object($value)) {
                    $_data[$key] = '<span class="rocketmail-key" title="Variable">'.$key.'</span>';
                }
            }
        } else {
            $_data = [];
